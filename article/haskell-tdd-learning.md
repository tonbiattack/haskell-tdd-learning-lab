...
Haskellの学習を構文の暗記で終わらせず、テストを仕様として扱いながら、純粋関数と型による設計へ進む教材を作りました。この記事では、学習方針を一つの実行可能なCabalプロジェクトへ落とし込み、Red → Green → Refactorの流れを、FizzBuzz、`Maybe`、`Either`、スマートコンストラクタ、IO分離、ファイルI/O、関数による依存性注入、QuickCheckで確認します。
...
| スマートコンストラクタ | 検証済みの名前・年齢を構築する | 不正状態をいつ排除するか |
| ファイルI/O | TODOをTSVとして保存・読込する | 純粋な変換と外部境界をどう分けるか |
| 依存性注入 | 出力関数を引数で受け取る | テストで依存をどう差し替えるか |
...
## スマートコンストラクタ・ファイルI/O・依存性注入へ広げる

入力検証を`Either`で返すだけでなく、空でない名前と範囲内の年齢だけを構築できる`UserName`と`Age`を追加しました。`mkUserName`と`mkAge`を通った値だけを`ValidRegistration`へ入れることで、後続の関数は「検証済み」という前提を型で受け取れます。

TODO保存では、`renderTodos`と`parseTodos`を純粋関数へ残し、`saveTodos`と`loadTodos`だけをIOにします。純粋な往復性をまずHspecで確認し、最後に一時ファイルを使って保存・読込が接続されることを検証します。出力依存は`String -> IO ()`として`runReport`へ渡し、テストでは`IORef`への手書きスパイへ差し替えます。

この三つの章により、型による不変条件、実ファイルを使う最小の統合テスト、外部依存を取り替える単位テストを、基本の純粋関数から連続した学習順序で扱います。
...
次の発展課題は、TSVで表現できない文字を含むタイトルの形式選定、JSONへの移行、ソートの性質、HTTP境界、非同期処理です。どの課題でも、最初に仕様テストを書き、失敗を確認してから実装を始めます。
